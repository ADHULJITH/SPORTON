...
def turf_chat_with_users():
	if not session.get("lid") is None:
		data={}
		q="SELECT *,concat(first_name,' ',last_name) as NAME FROM `users` WHERE user_id IN (SELECT IF(`sender_id`='%s' AND sender_type='turf',`receiver_id`,`sender_id`) FROM chat WHERE sender_id='%s' AND sender_type='turf' OR receiver_id='%s' AND receiver_type='turf')"%(session['lid'],session['lid'],session['lid'])
		# q="SELECT *,CONCAT(first_name,' ',last_name) AS NAME FROM  `users` INNER JOIN `login` USING(`login_id`) INNER JOIN  chat ON `chat`.`receiver_id`=`users`.`user_id` WHERE sender_id='%s' OR receiver_id='%s' group by sender_id "%(session['lid'],session['lid'])
		print("--------------",q)
		res=select(q)
		data['msgs']=res
...
def turf_view_bookings():
	if not session.get("lid") is None:
		data={}
		qq = "SELECT COUNT(book_id) AS ct FROM bookings WHERE date_time >= DATE_SUB(now(), INTERVAL 7 DAY) AND date_time <= now()"
		r = select(qq)
		if r:
			print("/////////////////////////",r[0]['ct'])
			data['ct']=r[0]['ct']
		...
		if 'action' in request.args:
			...
			action=None
		if action=='accept':
			up="update bookings set status='accept' where book_id='%s'"%(book_id)
			update(up)
			return redirect(url_for('turf.turf_view_bookings'))
		if action=='reject':
			up="update bookings set status='already booked' where book_id='%s'"%(book_id)
			update(up)
			return redirect(url_for('turf.turf_view_bookings'))
...
def turf_match_detail():
	if not session.get("lid") is None:
		...
		id=request.args['id']
		q="SELECT *,concat(first_name,' ',last_name)as name,bookings.date_time as dt FROM matches INNER JOIN bookings USING(book_id) INNER JOIN users ON users.user_id=matches.`opp_user_id` inner join slots using(slot_id) where match_id='%s' and matches.status='accepted'"%(mid)
		data['match']=select(q)
		q="select *,concat(first_name,' ',last_name)as name from users where user_id='%s'"%(id)
		data['user']=select(q)
...
def turf_chat_message():
	if not session.get("lid") is None:
		...
		if 'submit' in request.form:
			...
			q2="INSERT INTO chat(sender_id,sender_type,receiver_id,receiver_type,message,`date_time`)VALUES('%s','turf','%s','user','%s',NOW())"%(session['tid'],cid,message)
			insert(q2)
			return redirect(url_for('turf.turf_chat_message',cid=cid))
		q="SELECT * FROM chat WHERE (`sender_id`='%s' AND sender_type='turf' AND `receiver_id`='%s') OR (`sender_id`='%s' AND `receiver_id`='%s' AND receiver_type='turf')"%(session['tid'],cid,cid,session['tid'])
		res=select(q)
		data['msg']=res
...
def turf_gallery():
	if not session.get("lid") is None:
		...
		q="SELECT * FROM images WHERE image_type='%s' and turf_id='%s'"%(image_type,session['tid'])
		data['img']=select(q)
...
@turf.route("/turf_view_payment")
def turf_view_payment():
	if not session.get("lid") is None:
		data={}
		book_id=request.args['book_id']
		q="SELECT *,concat(first_name,' ',last_name)as name FROM payments inner join bookings on bookings.book_id = payments.paymment_for_id inner join users using(user_id) WHERE book_id='%s'"%(book_id)
		data['pay']=select(q)
