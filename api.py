...
# 	q="SELECT *,(3959 * ACOS ( COS ( RADIANS('%s') ) * COS( RADIANS( latitude) ) * COS( RADIANS( longitude ) - RADIANS('%s') ) + SIN ( RADIANS('%s') ) * SIN( RADIANS(latitude ) ))) AS user_distance  FROM `ambulance` HAVING user_distance<31.068 " % (lati,logi,lati)
...
def ViewTurf():
	...
	logi=request.args['logitude']
	# q="select *,concat(turfs.owner_first_name,'',turfs.owner_last_name) as owner_name from turfs "
	q="SELECT *,concat(turfs.owner_first_name,'',turfs.owner_last_name) as owner_name,(3959 * ACOS ( COS ( RADIANS('%s') ) * COS( RADIANS( latitude) ) * COS( RADIANS( longitude ) - RADIANS('%s') ) + SIN ( RADIANS('%s') ) * SIN( RADIANS(latitude ) ))) AS user_distance  FROM `turfs` inner join login using(login_id) where usertype = 'turf' HAVING user_distance<31.068  " % (lati,logi,lati)
	res=select(q)
	if res:
		data['data']=res
...
def turfgallery():
	...
	print('XXXXXXXXXXxxxxxxxxxxxxxxxx',q)
	res=select(q)
	if res:
		data['status']="success"
...
def MyMatches():
	...
	# q="SELECT *,CONCAT(users.first_name,'',users.last_name) AS user_name,CONCAT(turfs.owner_first_name,'',turfs.owner_last_name) AS owner_name FROM bookings INNER JOIN slots USING(slot_id) INNER JOIN turfs USING(turf_id) INNER JOIN matches USING(book_id) INNER JOIN users ON users.user_id=matches.opp_user_id WHERE `matches`.`opp_user_id`='%s' or "%(res1[0]['user_id'])
...
# @api.route('/send_weather',methods=['get','post'])
# def send_weather():
# 	data={}
# 	cityEditText=request.args['cityEditText']
# 	q="insert into payments values(NULL,'%s','coach','online',now(),'%s')"%(rid,amount)
# 	insert(q)
# 	q="update `request` set status='paid' where request_id='%s'"%(rid)
# 	update(q)
...
# def send_weather():
# 	data = {}
# 	api_key = "8d6570b8d9ce6ab74dab4836a22ba6fe"

# 	cityEditText=request.args['cityEditText']
# 	print("............................",cityEditText)
# 	out= get_weather(cityEditText,api_key)
# 	print(out)
# 	if out:
# 		data['status'] = 'success'
...
def send_weather():
	...
	if out:
		...
		if out <= 23.0:
			ss="rainy day today"
			if ss == "rainy day today":
				q="SELECT * FROM `turfs` WHERE `turf_type`='indoor'"
				res=select(q)
				data['data']=res
		else:
			ss= "normal condition"
			if ss == "normal condition":
				q="SELECT * FROM `turfs` WHERE `turf_type`='outdoor'"
				res=select(q)
				data['data']=res
...
@api.route('/User_view_all_turf')
def User_view_all_turf():
	data={}
	q="select * from `turfs`"
	res=select(q)
	if res:
		...
		data['status']="failed"
	data['method']="User_view_all_turf"
	return demjson.encode(data)